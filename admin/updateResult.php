...
if( isset($_POST['update']) ) {
    ...
    $db->query( "UPDATE student_company SET selected = 'yes' WHERE id IN (".implode(",", $selected ) . ") "  );
...
if(!empty($_POST) && !isset($_POST['update'])){
    ...
    $qryPlaced = $db->query("SELECT * FROM student_company WHERE comp_id = '$comp_id'");
            ...
            <div class="container-fluid">
               <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php if(!empty($_POST) && !isset($_POST['update'])) : ?>
                        <div class="card">
                            <div class="header">
                                <h4 class="title">Select Students Placed in <?= $company_name ;?></h4>
                            </div>                            
                            <div class="content">    
                                <form action="updateResult.php" method="post">
                                    <div class="table-full-width">
                                        <table class="table">
                                            ...
                                            <tbody> 
                                                ...
                                                ?>  
                                                <tr>
                                                    <td><input type="checkbox" name="checkbox[]" value="<?=$resultPlaced['id'];?>"></td>
                                                    <td><?=$studentDetails['full_name'];?></td>
                                                    <td><?=$studentDetails['roll_no'];?></td>
                                                    <td><?=$resultPlaced['job_position'];?></td> 
                                                    <td><?=$resultPlaced['selected'];?></td>
                                                        <!-- <td>
                                                            <a href="updateResult.php?id=<?=$resultPlaced['id'] ?>"><button type="button" class="btn btn-success btn-fill btn-xs">Selected</button>
                                                                    </a>
                                                        </td> -->
                                                </tr>
                                                <?php endwhile ;?>                                     
                                ...
                                <button type="submit" name="update" class="btn btn-success btn-fill">Update Result</button> 
                                </form>     
                            </div>                                                          
                        </div>
                        <?php endif ; ?>
